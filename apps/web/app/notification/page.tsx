...
export default function NotificationPage() {
  ...
  const trigger = useCallback<TriggerFn>(
    (title, body, duration, action) => {
      ...
      setData({ title, body, duration: effectiveDuration, action });
      setProgress(100);
  ...
  const handleAction = useCallback(
    (event: MouseEvent<HTMLButtonElement>) => {
      ...
      if (data?.action?.type === "markPrayerPrayed") {
        markPrayerAsPrayed(data.action.prayer);
      }
      hideWindow();
    },
    [data, hideWindow],
      ...
      {data.action && (
        <div className="px-4 pb-4" style={{ background: bgColor }}>
          <button
            ...
            className="flex h-10 w-full items-center justify-center gap-2 rounded-lg border text-sm font-black transition-colors hover:opacity-90"
            style={{
              borderColor: settings.appearance.headerBgColor + "40",
              backgroundColor: settings.appearance.headerBgColor + "18",
              color: settings.appearance.headerBgColor,
            }}
          >
