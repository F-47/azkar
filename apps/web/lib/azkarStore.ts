...
export function deleteCustomZekr(id: number): void {
  ...
  const notifDisabled = getNotifDisabledIds().filter(disabledId => disabledId !== id)
...
export function toggleAllZekrNotifStatus(ids: number[], enabled: boolean): void {
  let disabled = getNotifDisabledIds()
  if (enabled) {
    disabled = disabled.filter(disabledId => !ids.includes(disabledId))
  } else {
